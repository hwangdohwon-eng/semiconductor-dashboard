...
if system == "Windows":
    ...
    try:
        ...
        plt.rcParams["font.family"] = "NanumGothic" if "NanumGothic" in names else "DejaVu Sans"
    except:
...
# ── 세션 초기화 ──────────────────────────────────────────────
def init():
    for k,v in {"df_eqp":None,"df_thk":None,"df_coor":None,"df_recipe":None,"run_cols":[],"eqp_num_cols":[]}.items():
...
with st.sidebar:
    ...
    f_recipe = st.file_uploader("📋 레시피 데이터 [선택]",            type=["xlsx","xls"], key="fu_rec")
        ...
        with st.spinner("데이터 불러오는 중..."):
            try:
                if f_thk:
                    ...
                    run_cols = [c for c in df_t.columns if any(k in str(c) for k in ["Run","run","Prerun","prerun"])]
                    for c in run_cols:
                ...
                if f_eqp:
                    ...
                    useful = [c for c in num_cols if not any(x in c.lower() for x in ["id","no","number","count","index"])]
...
if menu == "🏠 홈 — 데이터 개요":
    ...
    ax.axhline(np.mean(means), color="red", linestyle="--", label=f"전체 평균: {np.mean(means):.1f} Å")
    ax.set_xticks(range(len(run_cols)))
    ax.set_xticklabels(run_cols, rotation=45, ha="right")
    ax.set_ylabel("Thickness (Å)"); ax.set_title("Run별 평균 두께 추이")
    ...
    for c in run_cols:
        v = df_thk[c].dropna()
        rows.append({"Run":c,"평균(Å)":round(v.mean(),1),"표준편차":round(v.std(),2),
                     "최소":round(v.min(),1),"최대":round(v.max(),1),
                     "범위":round(v.max()-v.min(),1),"균일도(%)":round(v.std()/v.mean()*100,3)})
...
# ══════════════════════════════════════════════════════════════
    ...
    with col2:
        ...
        filled = ax.tricontourf(triang, z, levels=np.linspace(vmin,vmax,200), cmap=colormap, vmin=vmin, vmax=vmax)
        if show_contour:
            cl = ax.tricontour(triang, z, levels=np.linspace(vmin,vmax,15), colors="black", linewidths=0.7)
            if show_labels:
        ...
        cb.set_label("Thickness (A)", fontsize=8)
...
# ══════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        ax.axvline(vals.mean()-vals.std(), color="orange", linestyle=":", label="-1σ")
        ax.set_xlabel("Thickness (A)"); ax.set_ylabel("Count")
        ax.set_title(f"{selected_run} Thickness Distribution"); ax.legend()
        plt.tight_layout(); st.pyplot(fig); plt.close()

    with col2:
        fig, ax = plt.subplots(figsize=(6,4))
        stats.probplot(vals, dist="norm", plot=ax)
        ax.set_title("Normal Probability Plot (Q-Q)")
        plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    ax.set_xticks(range(1, len(run_cols)+1))
    ax.set_xticklabels(run_cols, rotation=45, ha="right")
    for patch in bp["boxes"]:
        patch.set_facecolor("lightsteelblue")
    ax.set_xlabel("Run"); ax.set_ylabel("Thickness (A)")
    ax.set_title("Run별 Thickness Distribution (Boxplot)")
    plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    if p_val > 0.05:
        ...
        st.warning(f"📐 Shapiro-Wilk 정규성 검정 [{selected_run}] : W={stat_val:.4f}, p={p_val:.4f} → 정규분포 아닐 수 있음 ⚠️")
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        sns.heatmap(df_runs.corr(), annot=True, fmt=".2f", cmap="coolwarm", vmin=-1, vmax=1, ax=ax, annot_kws={"size":8})
        ax.set_title("Run Correlation Heatmap")
        plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    with tab2:
        if df_eqp is None or not eqp_num_cols:
            ...
            sel_params = st.multiselect("분석할 파라미터 선택", eqp_num_cols, default=eqp_num_cols[:6])
            if sel_params:
                n = min(len(df_eqp), len(df_thk))
                df_merge = df_eqp[sel_params].iloc[:n].copy().reset_index(drop=True)
                df_merge["thickness"] = df_thk[target_run].values[:n]
                df_merge = df_merge.dropna()
                fig, ax = plt.subplots(figsize=(10,8))
                sns.heatmap(df_merge.corr(), annot=True, fmt=".2f", cmap="coolwarm", vmin=-1, vmax=1, ax=ax)
                ax.set_title(f"Parameter vs {target_run} Correlation")
                plt.tight_layout(); st.pyplot(fig); plt.close()
                st.info("💡 두께와 상관계수 절댓값이 클수록 해당 파라미터가 두께에 큰 영향을 미칩니다.")
...
# ══════════════════════════════════════════════════════════════
    ...
    ax1.bar(df_s["Run"], df_s["F-통계량"], color=colors, edgecolor="black", alpha=0.85, label="F-statistic")
    ax1.set_ylabel("F-statistic"); ax1.set_xlabel("Run")
    ax1.set_title(f"ANOVA F-statistic / p-value (ref: {ref_run})")
    ax2 = ax1.twinx()
    ax2.plot(df_s["Run"], df_s["p-value"], color="orange", marker="o", lw=2, label="p-value")
    ax2.axhline(0.05, color="red", linestyle="--", lw=1.5, label="p=0.05")
...
# ══════════════════════════════════════════════════════════════
    ...
    ax.axhline(1.33, color="green",  linestyle="--", label="Cpk=1.33 (excellent)")
    ax.axhline(1.0,  color="orange", linestyle="--", label="Cpk=1.0  (minimum)")
    ...
    st.info("📌 **Cp vs Cpk** : Cp는 공정 능력 자체, Cpk는 공정이 목표값에 얼마나 잘 맞는지를 반영합니다. **Cp는 높은데 Cpk가 낮으면** 공정이 한쪽으로 치우쳐 있다는 뜻입니다.")
...
# ══════════════════════════════════════════════════════════════
    ...
    ax.set_xlabel("Data Index"); ax.set_ylabel("Value")
    ...
    with col_a:
        st.success("✅ 정상 패턴")
        st.markdown("- Actual(점)이 Set(빨간선)에 촘촘하게 붙어 있음\n- 오차가 일정하고 작음\n- 초반 안정화(settling) 후 일정하게 유지")
    with col_b:
        st.error("⚠️ 이상 패턴")
        st.markdown("- Actual이 Set에서 점점 멀어지는 추세 → **장비 드리프트**\n- 특정 구간에서 갑자기 튀는 값 → **공정 이상 이벤트**\n- 진동(oscillation) 패턴 → **제어 파라미터 불안정**")
...
# ══════════════════════════════════════════════════════════════
    ...
    df_p["status"] = df_p["thickness"].apply(
        lambda v: "High" if v>mu+threshold*sig else ("Low" if v<mu-threshold*sig else "Normal"))

    color_map = {"Normal":"green","High":"red","Low":"blue"}
    with col2:
        fig, ax = plt.subplots(figsize=(6,6))
        for status, grp in df_p.groupby("status"):
        ...
        ax.set_aspect("equal")
        ax.set_title(f"{selected_run} Defect Pattern (±{threshold}σ)")
        ax.set_xlabel("X (mm)"); ax.set_ylabel("Y (mm)")
        ax.legend(); ax.grid(True, alpha=0.2); plt.tight_layout()
        st.pyplot(fig); plt.close()

    good = (df_p["status"]=="Normal").sum()
    bad  = (df_p["status"]!="Normal").sum()
    ...
    with col_a:
        ...
        st.markdown("웨이퍼 외곽에 불량 집중\n\n→ 가스 흐름 불균일\n→ 히터 온도 분포 문제\n→ Edge ring 마모 의심")
    with col_b:
        st.error("🔴 중심부(Center) 집중")
        st.markdown("웨이퍼 중심에 불량 집중\n\n→ 샤워헤드 막힘\n→ 플라즈마 밀도 불균일\n→ 가스 유량 조정 필요")
    with col_c:
        st.error("🔴 한쪽 치우침(Asymmetry)")
        st.markdown("불량이 한쪽으로 치우침\n\n→ 웨이퍼 기울어짐(tilt)\n→ 가스 편류(gas drift)\n→ 척(chuck) 수평 점검 필요")
